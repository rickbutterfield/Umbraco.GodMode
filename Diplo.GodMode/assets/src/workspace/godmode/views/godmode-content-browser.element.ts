...
export class GodModeContentBrowserElement extends UmbElementMixin(LitElement) {
    ...
    async #loadFilterData() {
        ...
        const { data: aliases } = await tryExecute(this, GodModeService.getContentTypeAliases());
        if (aliases) {
        ...
        // Load languages
        const { data: langs } = await tryExecute(this, GodModeService.getLanguages());
        if (langs) {
    ...
    async #fetchContent() {
        ...
        const { data } = await tryExecute(this, GodModeService.getContentPaged({ query }));
