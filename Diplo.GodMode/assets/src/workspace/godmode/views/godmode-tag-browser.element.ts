...
export class GodModeTagBrowserElement extends UmbElementMixin(LitElement) {
    ...
    async #loadTags() {
        ...
        const { data: tagMappingData } = await tryExecute(this, GodModeService.getTagMapping());
        const { data: orphanedData } = await tryExecute(this, GodModeService.getOrphanedTags());
    ...
    async #deleteTag(tagId: number, tagText: string) {
        ...
        const { data } = await tryExecute(this, GodModeService.deleteTag({ query: { id: tagId } }));
